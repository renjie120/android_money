package com.renjie.treelistview;
...
public abstract class AbstractTreeViewAdapter<T> extends BaseAdapter implements
	...
	public AbstractTreeViewAdapter(final Activity activity,
		...
		this.itemPadding = (int) TypedValue.applyDimension(
	...
	protected int getItemWidth() {
		return getIndentWidth() * (isCollapsible() ? (maxLevel + 1) : maxLevel)
				+ getLayoutWidth() + itemPadding * 2;
	}
	...
	public final View getView(final int position, View convertView,
		...
		if (convertView == null || (isAutoAdapt && maxLevel != oldMaxLevel)) {
			...
			convertView = layoutInflater.inflate(getTreeListItemWrapperId(),
					null);
			oldMaxLevel = maxLevel;
			return populateTreeItem(convertView, getNewChildView(nodeInfo),
					nodeInfo, true);
		} else {
	...
	/**
	 * Retrieves background drawable for the node. 设置每个节点的布局样式.
	 * 
	 * @param treeNodeInfo
	 *            node info
	...
	public abstract Drawable getBackgroundDrawable(
			final TreeNodeInfo<T> treeNodeInfo);

	public final View populateTreeItem(View view, View childView,
			final TreeNodeInfo<T> nodeInfo, final boolean newChildView) {
		System.out.println("populateTreeItem===" + nodeInfo.getId());
		...
		if (isAutoAdapt) {
			LinearLayout.LayoutParams layoutParams = (LinearLayout.LayoutParams) layout
					.getLayoutParams();
			layoutParams.width = getItemWidth();
		}
		LinearLayout indicatorLayout = (LinearLayout) view
		...
		System.out.println("nodeInfo.isWithChildren() ==="
				+ nodeInfo.isWithChildren() + ",,collapsible=" + collapsible
				+ ",calculateIndentation(nodeInfo)="
				+ calculateIndentation(nodeInfo)+",nodeInfo.getLevel() ="+nodeInfo.getLevel() );
		...
		FrameLayout frameLayout = (FrameLayout) view
				.findViewById(R.id.treeview_list_item_frame);
		System.out.println("populateTreeItem---newChildView=" + newChildView);
		if (newChildView) {
